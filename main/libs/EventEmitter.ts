export class EventEmitter {
    private events: { [key: string]: { callback: Function, scope: any, once: boolean }[] } = {};

    on(event: string, callback: Function, scope: any = null, once: boolean = false) {
    ...
    off(event: string, callback: Function, scope: any = null) {
        ...
        this.events[event] = this.events[event].filter(item => item.callback !== callback || item.scope !== scope);
    }
